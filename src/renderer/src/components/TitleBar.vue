...
<template>
  <header
    class="title-bar drag-region flex items-center justify-between h-10 px-4 bg-[var(--color-surface-light)] border-b border-[var(--color-border)]"
  >
    <!-- 左侧 Logo -->
    <button
    ...
    >
      <div
        class="logo flex items-center justify-center w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600"
      >
    ...
    <div class="window-controls no-drag flex items-center gap-1">
      ...
      >
        ...
        >
          ...
          <path
            stroke-linecap="round"
            stroke-width="2"
            d="M9 6V5a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2h-1"
          />
        </svg>
      </button>
      <button
      ...
      >
        <svg
          class="w-4 h-4 text-[var(--color-text-muted)] group-hover:text-white"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
